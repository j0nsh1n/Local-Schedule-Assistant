...
s = app.now_next_summary(day, 560)
...
s = app.now_next_summary(day, 600)
...
expect = app.now_next_summary(mw._all_acts, now_min)
mw._update_nownext()
...
check("tooltip has version line", mw._tray.tip.splitlines()[0] == f"Daily Scheduler v{app.APP_VERSION}")
if expect:
    check("tooltip includes now/next", expect in mw._tray.tip)
else:
    check("tooltip has no now/next when idle", app.APP_VERSION in mw._tray.tip)
